...
function BaseInvoke:Connect(Callback): ()
	assert(typeof(Callback) == "function", self._errSrc .. "Passed wrong first argument into :Connect(Callback: (...) -> (...)). Got " .. typeof(Callback))
...
function BaseInvoke:Invoke(Player, ...) --> Promise
	if self._checkForPlayer then
		assert(typeof(Player) == "Instance" and game.IsA(Player, "Player"), self._errSrc .. "Passed wrong first argument into :Invoke(Player: Player, ...). Got " .. typeof(Player))
	end
	local args = {Player, ...}
...
function BaseInvoke:InvokeAsync(Player, ...) --// If you feel like yielding
	if self._checkForPlayer then
		assert(typeof(Player) == "Instance" and game.IsA(Player, "Player"), self._errSrc .. "Passed wrong first argument into :InvokeAsync(Player: Player, ...). Got " .. typeof(Player))
	end
