--// By evaera
--// The module was adjusted a bit to fit PascalCase style
...
Promise.prototype = {}
Promise.prototype.ClassName = "Promise"
...
function Promise.new(executor, ...)
	return Promise._new(debug.traceback(nil, 2), executor, nil, ...)
end
...
function Promise.prototype:catchCall(callback, ...)
	assert(type(callback) == "function", string.format(ERROR_NON_FUNCTION, "Promise:catchCall"))
	local length, values = pack(...)
	return self:_andThen(debug.traceback(nil, 2), nil, function()
		return callback(unpack(values, 1, length))
	end)
end

--[[function Promise.prototype:catchTap(tapCallback, ...)
	local args = {...}
	assert(type(tapCallback) == "function", string.format(ERROR_NON_FUNCTION, "Promise:catchTap"))
	return self:_andThen(debug.traceback(nil, 2), nil, function(...)
		local callbackReturn = tapCallback(..., unpack(args))
...
Promise.prototype["then"] = Promise.prototype.andThen --// :Then() support
Promise.prototype.thenCall = Promise.prototype.andThenCall
Promise.prototype.thenReturn = Promise.prototype.andThenReturn

for _, dir in pairs({Promise, Promise.prototype}) do  --// PascalCase support
	for name, variable in pairs(dir) do
		local firstLeter = string.sub(name, 1, 1)
		local rest = string.sub(name, 2)
	
		dir[string.upper(firstLeter)..rest] = variable
	end
end
